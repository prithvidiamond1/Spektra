...
void AudioLoopBack::ALB_contextAndDeviceInit()
{
...
{
	ma_result ALB_result = ma_device_start(&ALB_device);
	if (ALB_result != MA_SUCCESS) {
	...
	// main loop
	while (true) {
		void* ALB_deinterleavedLeftData = new float[ALB_latestFrameCount];
		void* ALB_deinterleavedRightData = new float[ALB_latestFrameCount];
		...
		ma_deinterleave_pcm_frames(ALB_captureFormat, ALB_captureChannelCount, ALB_latestFrameCount, ALB_device.pUserData, ALB_deinterleavedData);
		...
		std::vector<float> ALB_leftChannelData = std::vector<float>(ALB_castedLeftData, ALB_castedLeftData + ALB_latestFrameCount);
		std::vector<float> ALB_rightChannelData = std::vector<float>(ALB_castedRightData, ALB_castedRightData + ALB_latestFrameCount);
